...
export default class PreRegisterJobSelectionDropDown extends Component {
        ...
        return (
            <Item style={styles.input} >
                <Picker mode="dropdown" style={{ marginLeft: "10%", width: 270 }} selectedValue={that.state.selectedJob} onValueChange={this.props.onChangeFunc.bind(that)} >
                    <Items style={{ fontSize: 12 }} label={strInReqLan.selectJob} value={""} />
                    
                    {
                        this.props.jobListArr
                            .filter((unFilteredjobListArr) => {
                                return unFilteredjobListArr.type == "Empregos" || unFilteredjobListArr.type == "Jobs"
                            }).map((jobListArr, index) => {
                                console.log(jobListArr, "5555555555555")
                                return (
                                    <Items style={{ fontSize: 12 }} label={jobListArr.jobTitle} value={JSON.stringify(jobListArr)} key={index} />
                                )
                            })
                    }
                </Picker>
